...
class SettingsController extends Controller
{
    ...
    public function index()
    {
...

        {
            return Inertia::render('Settings/Show', [
                ...
                'storetime' => Settings::where('name', SettingsType::STORETIME())->first() ?? null,
    ...
    public function storetime(Request $request, Settings $settings)
    {
        if (request()->user()->cannot('update', $settings)) {
            abort(403);
        }
        ...
        DB::transaction(function () use ($request) {
            Settings::updateOrCreate(
                [
                    'name' => SettingsType::STORETIME()
                ],
                [
                    'data' => [
                        'open'        => $request->open,
                        'close'        => $request->close,
                        'prepration'      => $request->prepration,
                        'twentyfourhours'      => $request->twentyfourhours,
                        'hours'      => $request->hours,
                    ]
                ]
            );
