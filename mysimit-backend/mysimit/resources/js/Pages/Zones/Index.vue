<template>
	...
	<index-view title="Areas" :breadcrumb="breadcrumb">
		<datatable :data="zones" searchRoute="zones.index" :filters="filters">
			...
			<template #right-header>
				...
				<button-link class="px-6 py-3 ml-4" :href="route('zones.create')" v-if="$page.props.can.createZones">
					...
					<span class="hidden md:inline">Area</span>
				</button-link>
			</template>
				...
				<table v-if="rows.length">
					...
					<tbody>
						<tr v-for="(row, index) in rows" :key="index">
							...
							<td>
								<label class="toggle">
									<input type="checkbox" class="togglecheck"  @click="changeStatus(route('status.update', row.id))" :checked="row.status == 1">
...
export default {
	...
	methods:{
		toggle(){
			...
			if (button.innerHTML === "Toggle") {
				button.innerHTML = "Toggled";
				// Update the logic for the value change based on your requirements
			} else {
...
/* Adjust the background color and handle position when checked */
.togglecheck:checked + .slider {
  background-color: #2196F3;
}

.togglecheck:checked + .slider::before {
