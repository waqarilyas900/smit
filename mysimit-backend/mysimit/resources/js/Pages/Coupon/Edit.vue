<template>
	<form-view @submitted="update('coupons.update', coupon.id)" title="Edit Coupon" :breadcrumb="breadcrumb">
		<template #form>
		...
		<template #actions>
			...
			<jet-button @click.prevent="update('coupons.update', coupon.id, true)" class="px-6 xs:mr-2 my-2 xs:my-0" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">Update & Continue</jet-button>
	...
	data() {
		return {
			...
			form: this.$inertia.form({
                ...
                used: this.coupon.used,
                expiry: this.coupon.expiry,
			}),
