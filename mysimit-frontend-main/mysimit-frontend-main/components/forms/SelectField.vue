<template>
  <div>
    <label v-if="label" class="flex mb-1 text-sm">{{ label }}
      <span v-if="required" class="text-black-500">*</span>
    </label>
    <div class="flex items-center">
      <select class="flex border border-gray-300 w-full px-3 py-2 text-sm rounded cursor-pointer outline-none" :value="value" @input="$emit('input', $event.target.value)" :disabled="disabled" :class="{ 'bg-light': disabled }">
    ...
    <span class="text-black text-xs">{{ error[0] }}</span>
  </div>
</template>
