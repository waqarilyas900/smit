  ...
  <div>
    ...
    <div class="bg-white shadow-sm hover:shadow-lg border border-gray-100 hover:border-gray-200 rounded-lg overflow-hidden mx-auto relative product">
      ...
      <div class="bg-gray-100 flex justify-center item-center product-image relative">
        ...
        <div class="absolute bg-white left-3.5 overflow-hidden product-discount-badge px-3.5 py-1.5 top-4 rounded" v-if="product.discount">
        ...
        <div class="product-options absolute flex items-center justify-center">
          ...
          <!-- add to cart button -->
          <div @click="addToCart" class="ml-4 h-12 w-12 bg-white hover:bg-primary-500 rounded-full shadow-md flex justify-center items-center text-primary-500 hover:text-white cursor-pointer mr-4 hidden">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div class="details mt-5 text-center py-4">
        ...
        <nuxt-link :to="`/shop/${product.id}`" :product="product" class="text-md font-semibold mt-4">{{product.name[$i18n.locale]}}</nuxt-link>
        <!-- price -->
        <div class="mt-2 font-bold text-xl">
          <!-- <span class="text-primary-500">{{currencyFormat(product.salePrice)}}</span>
          <span class="line-through ml-2 text-black-100">{{currencyFormat(product.actualPrice)}}</span> -->
          <span class="text-primary-500">{{currencyFormat(product.actualPrice)}}</span>
        </div>
      </div>
